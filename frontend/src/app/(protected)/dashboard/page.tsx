...
export default function AdminDashboard() {
    ...
    return (
        <div className="p-4 md:p-8 space-y-8 w-full relative min-h-screen bg-gray-50/20">
            <header className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 relative z-10">
                ...
                <div className="flex flex-wrap items-center gap-2 bg-white/60 backdrop-blur-xl p-2 rounded-[1.5rem] shadow-xl shadow-gray-200/50 border border-white/40">
                    {(['today', '7days', '30days', 'custom'] as const).map((p) => (
                        <button
                            key={p}
                            onClick={() => setPeriod(p)}
                            className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${period === p
            ...
            {notifications.length > 0 && (
                <div className="space-y-3 z-10 relative">
                    {notifications.map((notif: any) => (
                        <div key={notif.id} className="bg-blue-50/80 backdrop-blur-md border border-blue-200 p-4 rounded-2xl flex items-start gap-4 shadow-sm animate-in fade-in slide-in-from-top-2">
            ...
            {period === 'custom' && (
                <div className="bg-white/40 backdrop-blur-2xl p-8 rounded-[2.5rem] shadow-2xl shadow-gray-100/50 border border-white/60 flex flex-wrap items-end gap-6 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="space-y-3">
                        ...
                        <input
                            ...
                            className="block w-full px-5 py-3.5 bg-white/80 border border-gray-100 rounded-2xl text-xs font-black focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all shadow-inner"
                        />
                    </div>
                    <div className="space-y-3">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {/* Ativos Quick Cards */}
                <div className="bg-white/60 backdrop-blur-xl p-6 rounded-[2rem] border border-white/60 shadow-lg flex items-center gap-4">
                ...
                {statCards.slice(0, 2).map((stat, idx) => (
                    <div key={idx} className="bg-white/60 backdrop-blur-xl p-6 rounded-[2rem] border border-white/60 shadow-lg flex items-center gap-4">
            ...
            <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Revenue Chart */}
                <div className="lg:col-span-2 bg-white/70 backdrop-blur-2xl p-8 rounded-[3rem] shadow-2xl border border-white/60 min-h-[400px] flex flex-col">
                            ...
                            <AreaChart data={chartData}>
                                <defs>
                                    <linearGradient id="colorTotal" x1="0" y1="0" x2="0" y2="1">
                                        <stop offset="5%" stopColor="var(--color-primary, #ff4d4d)" stopOpacity={0.3} />
                                        <stop offset="95%" stopColor="var(--color-primary, #ff4d4d)" stopOpacity={0} />
                                    </linearGradient>
                                </defs>
                                ...
                                <Area
                                    type="monotone"
                                    dataKey="total"
                                    stroke="var(--color-primary, #ff4d4d)"
                                    strokeWidth={4}
                                    fillOpacity={1}
                                    fill="url(#colorTotal)"
                                />
                ...
                {/* Top Products */}
                <div className="bg-white/70 backdrop-blur-2xl p-8 rounded-[3rem] shadow-2xl border border-white/60">
                    ...
                    <div className="space-y-4">
                        {loading ? (
                            ...
                            topProducts.map((prod, idx) => (
                                <div key={idx} className="flex items-center justify-between p-4 bg-white/40 rounded-2xl border border-white hover:border-primary/20 transition-all group">
            ...
            <section className="grid grid-cols-1 gap-8">
                ...
                <div className="bg-white/70 backdrop-blur-2xl rounded-[3rem] shadow-2xl border border-white/60 overflow-hidden">
                    <div className="p-8 border-b border-white/40 flex justify-between items-center">
                        ...
                        <Link href="/orders" className="text-[10px] font-black text-primary uppercase tracking-widest hover:underline">Ver Todos</Link>
                    </div>
            ...
            <div className="fixed bottom-0 left-0 w-[400px] h-[400px] bg-blue-500/5 rounded-full blur-[100px] -z-10 pointer-events-none"></div>
